...
**1. Data Capture**
#### Web (What applications on computer are being used)
  Script: Python monitors active windows.
  Storage: Data is saved to activity.db via SQLite.
  Sync: A cron job runs a script to upload the .db file to S3 using boto3.
...
This will write to an ontology with a singular object type session, with the backing dataset being the result of the UNION. 
...
**3. The final product will be a workshop**
...
Goal of Pipeline: Unify the web and browser data and normalize the schema so data quality is ensured. 
